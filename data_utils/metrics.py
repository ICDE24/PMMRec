...
def get_LMDB_only_scoring(model, item_num, item_id_to_keys, test_batch_size, args, local_rank):
    ...
    with torch.no_grad():
        for input_ids in item_dataloader:
            ...
            item_scoring.extend(item_emb)
            # item_scoring.extend(hidden_states_CV)
...
def eval_model(model, user_history, eval_seq, item_scoring, test_batch_size, args, item_num, Log_file, v_or_t,
    ...
    # Log_file.info(v_or_t + "_methods   {}".format('\t'.join(['Hit{}'.format(topK), 'nDCG{}'.format(topK)])))
    Log_file.info(v_or_t + "_methods   {}".format('\t'.join(['Hit{}'.format(10), 'nDCG{}'.format(10),'Hit{}'.format(20), 'nDCG{}'.format(20),'Hit{}'.format(50), 'nDCG{}'.format(50)])))
    item_scoring = item_scoring.to(local_rank)
    with torch.no_grad():
        eval_all_user = []
        eval_all_user_20 =[]
        ...
        for data in eval_dl:
            ...
            for user_id, label, score in zip(user_ids, labels, scores):
                ...
                score[history] = -np.inf
                score = score[1:]
                eval_all_user.append(metrics_topK(score, label, item_rank, 10, local_rank))
                eval_all_user_20.append(metrics_topK(score, label, item_rank, 20, local_rank))
                eval_all_user_50.append(metrics_topK(score, label, item_rank, 50, local_rank))

        eval_all_user = torch.stack(tensors=eval_all_user, dim=0).t().contiguous()
        Hit10, nDCG10 = eval_all_user
        eval_all_user_20 = torch.stack(tensors=eval_all_user_20, dim=0).t().contiguous()
        eval_all_user_50 = torch.stack(tensors=eval_all_user_50, dim=0).t().contiguous()
        Hit20, nDCG20 = eval_all_user_20
        Hit50, nDCG50 = eval_all_user_50
        # mean_eval = eval_concat([Hit10, nDCG10], test_sampler)
        mean_eval = eval_concat([Hit10, nDCG10,Hit20, nDCG20,Hit50, nDCG50], test_sampler)
        print_metrics(mean_eval, Log_file, v_or_t)
