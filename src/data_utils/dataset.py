...
class Build_text_CV_Dataset(Dataset):
    def __init__(self, u2seq, item_content, max_seq_len, item_num, text_size, db_path, item_id_to_keys,args,neg_sampling_list):
        self.u2seq = u2seq
    ...
    def __getitem__(self, index):
        ...
        sample_items_cv = np.zeros((self.max_seq_len, 3, self.resize, self.resize))
        ...
        with env.begin() as txn:
            for i in range(tokens_Len):
                ...
                sample_items_cv[mask_len_head + i] = image_trans

            # target
            IMAGE = pickle.loads(txn.get(self.item_id_to_keys[seq[-1]]))
            image_trans = self.transform(Image.fromarray(IMAGE.get_image()).convert('RGB'))
            sample_items_cv[mask_len_head + tokens_Len] = image_trans

        return sample_items_id, torch.FloatTensor(sample_items_text), torch.FloatTensor(sample_items_cv), torch.FloatTensor(log_mask)
...
class Build_Lmdb_Dataset(Dataset):
    def __init__(self, u2seq, item_num, max_seq_len, db_path, item_id_to_keys,args, neg_sampling_list):
...
class Build_Text_Dataset(Dataset):
    def __init__(self, userseq, item_content, max_seq_len, item_num, text_size,args, neg_sampling_list):
...
class Build_Id_Dataset(Dataset):
    def __init__(self, u2seq, item_num, max_seq_len, args, neg_sampling_list):
        self.u2seq = u2seq
        self.item_num = item_num
        self.max_seq_len = max_seq_len + 1 
        self.neg_sampling_list = neg_sampling_list
    ...
    def __getitem__(self, user_id):
        ...
        return sample_items_id,  torch.FloatTensor(log_mask)
...
# Multi-modal, enter text and img at the same time
